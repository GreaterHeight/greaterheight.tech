...
// Contact form: create a pre-filled email draft without pretending that a server endpoint exists.
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    const href = `mailto:sales@greaterheight.tech?subject=${encodeURIComponent(get("subject"))}&body=${encodeURIComponent(body)}`;
    const status = document.getElementById("contact-form-status");
    if (status) status.textContent = "Opening your email application with the message pre-filled.";
